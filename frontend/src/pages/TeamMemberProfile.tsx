    ...
    <DashboardLayout>
      ...
      <div style={styles.metricsRow}>
        <div style={styles.metricCard}>
          <div style={styles.metricValue}>{stats.totalReports}</div>
          <div style={styles.metricLabel}>Total Reports</div>
        </div>
        <div style={styles.metricCard}>
          <div style={styles.metricValue}>{stats.approvedCount}</div>
          <div style={styles.metricLabel}>Approved</div>
        </div>
        <div style={styles.metricCard}>
          <div style={styles.metricValue}>{stats.needsCorrectionCount}</div>
          <div style={styles.metricLabel}>Needs Correction</div>
        </div>
        <div style={styles.metricCard}>
          <div style={styles.metricValue}>{stats.complianceRate}%</div>
              ...
              {reports.map((r: any) => {
                ...
                return (
                  <tr key={r.id}>
                    ...
                    <td style={styles.td}>
                      <Link to={`/reports/${r.id}`} style={styles.link}>View</Link>
                    </td>
                  </tr>
                );
              })}
...
const styles: { [key: string]: React.CSSProperties } = {
  ...
  email: { color: "#666", margin: "0.25rem 0" },
  roleBadge: { background: "#e0e7ff", color: "#3730a3", padding: "0.2rem 0.6rem", borderRadius: "10px", fontSize: "0.8rem" },
  metricsRow: { display: "flex", gap: "1rem", marginBottom: "1.5rem" },
  metricCard: { flex: 1, background: "#fff", padding: "1rem", borderRadius: "8px", boxShadow: "0 2px 8px rgba(0,0,0,0.08)", textAlign: "center" },
  metricValue: { fontSize: "1.6rem", fontWeight: 700 },
  metricLabel: { fontSize: "0.8rem", color: "#666" },
  section: { background: "#fff", padding: "1.5rem", borderRadius: "8px", boxShadow: "0 2px 8px rgba(0,0,0,0.08)" },
  ...
  badge: { padding: "0.2rem 0.5rem", borderRadius: "10px", fontSize: "0.75rem" },
